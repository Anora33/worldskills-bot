...
def add_user(tid, fullname, phone, profession, language="uz"):
    try:
        ...
        c.execute("INSERT OR REPLACE INTO users VALUES(?,?,?,?,?,?,?,?)",
                 (str(tid), fullname, phone, profession, language, 0, "pending", None))
        conn.commit()
        conn.close()
        return True
    except: return False
...
# ✅ To'g'ri try/except bloki:
try:
    from bot.handlers import start, ai_chat, admin, admin_help, admin_review, webapp
